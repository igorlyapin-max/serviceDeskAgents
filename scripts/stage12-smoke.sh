...
assert "contract_source_lang" in js, js[:300]
assert "contract_language" in js, js[:300]
assert "contract_diagnostics" in js, js[:300]
assert "Диагностика локали OpenAPI" in js, js[:300]
assert "Нельзя удалить: сначала уберите связи" not in js, js[:300]
...
assert "Нельзя отвязать: сначала уберите связи" not in js, js[:300]
...
assert "Параметры шагов" in js, js[:300]
assert "Доступные ссылки на результаты" in js, js[:300]
assert "Имя сущности результата" not in js, js[:300]
assert "entity:&lt;сущность&gt;.&lt;поле&gt;" not in js, js[:300]
...
assert ".topbar" in css and ".notice" in css and "top: 56px" in css, css[:300]
